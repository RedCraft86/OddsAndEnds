...
public class ImprovedBoneMeal {
    ...
    public static void handleBoneMeal(Level level, Player player, BlockPos originPos) {
        float shortGrass = Mth.clamp(
                (float)CommonCfg.SHORT_GRASS_CHANCE.get() / 100.0f,
                0.0f, 100.0f
        );
        float tallGrass = Mth.clamp(
                (float)CommonCfg.SHORT_GRASS_CHANCE.get() / 100.0f,
                0.0f, 100.0f
        );
        ...
        level.getServer().tell(new TickTask(level.getServer().getTickCount(), () -> {
            ...
            while (it.hasNext()) {
                ...
                if (block == Blocks.GRASS && RANDOM.nextFloat() > shortGrass) {
    ...
    private static Block airOrRandomFlower() {
        float chance = Mth.clamp(
                (float)CommonCfg.RANDOM_FLOWER_CHANCE.get() / 100.0f,
                0.0f, 100.0f
        );
